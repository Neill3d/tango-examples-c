...
void Scene::InitGLContent(AAssetManager* aasset_manager) {
  ...
  camera_ = new tango_gl::Camera();
    cameraTop_ = new tango_gl::Camera();
    cameraHUD_ = new tango_gl::Camera();
  ...
  dynamic_texture_ = new tango_gl::DynamicTexture();
  foreground_mesh_ = tango_gl::meshes::MakePlaneMesh(1.0f, 1.0f, 1.0f);
  foreground_material_ = new tango_gl::Material();
  foreground_material_->SetShader(
          tango_gl::shaders::GetScreenVertexShader().c_str(),
          tango_gl::shaders::GetScreenFragmentShader().c_str());
  foreground_material_->SetParam("texture", dynamic_texture_);

  // Cube: Mesh, Material and Texture
  cube_mesh_ = tango_gl::meshes::MakeCubeMesh(1.0f);
  logo_texture_ = new tango_gl::Texture(aasset_manager, "aa_logo.png");
  ...
  // camera trajectory
  line_material_ = new tango_gl::Material();
  line_transform_.SetPosition(glm::vec3(0.0f, 0.0f, 0.0f));
...
void Scene::DeleteResources() {
  ...
  delete foreground_mesh_;
  foreground_mesh_ = nullptr;
  delete foreground_material_;
  foreground_material_ = nullptr;
  delete dynamic_texture_;
  dynamic_texture_ = nullptr;

  delete line_material_;
  line_material_ = nullptr;
}
...
void Scene::SetupViewPort(int w, int h) {
  ...
  lastAspectRatio = static_cast<float>(w) / static_cast<float>(h);
  camera_->SetAspectRatio(lastAspectRatio);


  cameraTop_->SetAspectRatio(static_cast<float>(w) / static_cast<float>(h));
  cameraTop_->SetProjectionMatrix(glm::ortho(0.0f, static_cast<float>(w), 0.0f, static_cast<float>(h), -1.0f, 1.0f));

    cameraHUD_->SetAspectRatio(static_cast<float>(w) / static_cast<float>(h));
    cameraHUD_->SetProjectionMatrix(glm::ortho(0.0f, static_cast<float>(w), 0.0f, static_cast<float>(h), -1.0f, 1.0f));

  //glm::quat q( glm::vec3(0.0f, 3.14f, 0.0f));

  //foreground_transform_.SetRotation(q);
    foreground_transform_.SetPosition(glm::vec3(0.5f * static_cast<float>(w), 0.5f * static_cast<float>(h), 0.0f));
    foreground_transform_.SetScale(glm::vec3(static_cast<float>(w), static_cast<float>(h), 1.0f));
    foreground_scale_init = glm::vec3(static_cast<float>(w), static_cast<float>(h), 1.0f);
...
void Scene::Render(const TangoPoseData& pose) {
  ...
  tango_gl::Render(*foreground_mesh_, *foreground_material_, foreground_transform_, *cameraHUD_);
    glDepthMask(GL_TRUE);
...
void Scene::RotateCubeByPose(const TangoPoseData& pose) {
  if (last_pose_timestamp_ > 0) {
    ...
    double delta_angle = delta_time * 20.0 * M_PI / 12.0;
  ...
  last_pose_timestamp_ = pose.timestamp;
    ...
    void Scene::RenderTrajectory(const std::vector<glm::vec3> &points) {

      tango_gl::RenderTrajectory(points, *line_material_, line_transform_, *camera_ );
