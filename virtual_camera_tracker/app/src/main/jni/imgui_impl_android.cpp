...
bool ImGui_ImplAndroidGLES2_CreateDeviceObjects()
{
    ...
    if (logLength > 0)
    {
        GLchar *log = (GLchar *)new GLchar[logLength];
...
bool ImGui_ImplAndroidGLES2_Init(AAssetManager* aasset_manager)
{
    ImGuiIO& io = ImGui::GetIO();

    // load a default font

    AAsset* asset = AAssetManager_open(aasset_manager, "roboto_medium.ttf", AASSET_MODE_BUFFER);
    if (asset != NULL) {

        size_t fileLength = AAsset_getLength(asset);
        char *fileContent = new char[fileLength+1];
        //const void *fileContent = AAsset_getBuffer(asset);
        if (nullptr != fileContent) {
            AAsset_read(asset, fileContent, fileLength);
            fileContent[fileLength] = 0;
            io.Fonts->AddFontFromMemoryTTF( (void*)fileContent, fileLength, 32.0f);
            //delete[] fileContent;
        }
        AAsset_close(asset);
    }
